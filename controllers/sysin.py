...
def index():
    ...
    form            = SQLFORM(db.sysin, idsysin, deletable=True)
    ...
    if  form.accepts(request.vars, session):
        if  ('delete_this_record' in request.vars) and \
            ...
            session.flash = 'Sysin Excluida'
            redirect(URL('index'))
        else:
            if  idsysin:
                session.flash = 'Sysin Alterada'
                redirect(URL('index', args=(idsysin)))
            else:
                session.flash = 'Sysin Incluída'
                redirect(URL('index'))
    if  session.flash:
    ...
    return dict(listdetail.index(['Sysin'],
                                  'sysin', sysin,
                                  query,
